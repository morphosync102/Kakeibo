...
### 2-4. POST レスポンスで更新後データを返す ⏸️ 見送り (2026-07-07)

- 方針: GAS の書き込み action が更新後の明細一覧(または差分)を返せば、
  書き込み後の再取得(コールドリード)を丸ごと省略できる。2-2 で体感が十分なら不要。
- GASデプロイ: **必要**。
- **見送り理由**: 2-2 の楽観的更新により、この案が解決するはずだった「書き込み後の待ち時間」は
  体感上ゼロになった(再取得はユーザーが気づかない裏で走る)。一方で実装には GAS の全書き込み
  action・Next.js プロキシ・クライアントの3層でレスポンス契約の変更が必要で、返却データと
  キャッシュの不整合リスクも伴う。知覚できない裏処理の削減に見合わないと判断。
  バックグラウンド再取得が別の理由(GAS 実行クォータ、通信量など)で問題化したら再検討する。
...
- テスト拡充: カテゴリ変更・削除フロー ✅ 対応済み (2026-07-07,
  `test/edit-actions-consistency.test.mjs`・`test/gas-transaction-update.test.mjs`)
- ローディング統一 ✅ 対応済み (2026-07-07, CalendarView / ManageView にスケルトン追加)
- Next.js 16 の新キャッシュ機構(cacheComponents 等)の活用 ⏸️ 見送り (2026-07-07)
  - **見送り理由**: 全画面がクライアントコンポーネントで、データ層には既にタグ付き fetch
    キャッシュ(1時間+書き込み時無効化)がある。cacheComponents はサーバーレンダリングされる
    コンテンツ向けの仕組みで、恩恵を受けるにはサーバーコンポーネント中心への作り直しが必要。
    遅さの根本原因は GAS のレイテンシであり、作り直しても速くならないため見送り。
